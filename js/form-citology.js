...
const validForm = (e) => {
    switch (e.target.name) {
        ...
        break;
        /*case "fur_date": // respuesta seguridad
            validarCampo(expressions.fecha, e.target, 'fur_date');
        break;*/
        case "other": // respuesta seguridad
            validarCampo(expressions.text, e.target, 'other');
        break;
    }
}
...
form.addEventListener('submit', (e) => {
    ...
    if (!document.querySelector('input[name="frotis"]:checked')) {
        alert('Error, selecciona una opción de "Frotis"');
        hasError = true;
        } else if (campos.entry_date && campos.resume && campos.doctor && campos.telf_doctor && campos.diagnosis && campos.fur_date){
        form.reset();
